...
export function parseArgs(argv) {
  const opts = { color: 'FF0000', off: false, keep: false, dryRun: false, debug: false, quota: false, watch: 0, beats: 0, mac: null, timeout: 20, pace: 40 };
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--off') opts.off = true;
      ...
      if (!Number.isFinite(opts.pace)) throw new Error(`--pace requires a number`);
    }
...
// bluetoothctl emits "[CHG] Device <MAC> Connected: no" when the link drops.
export function isDisconnected(line) {
  return /Connected:\s*no/i.test(stripAnsi(line));
}
...
export class BtCtl {
  ...
  // Register a persistent line listener; returns an unregister fn.
  onLine(fn) {
  ...
  // Draw a list of {x,y,r,g,b} pixels via graffiti setPixel (write-without-
  // response). Stays in DIY draw mode the whole time — no image upload, so the
  // panel updates incrementally instead of repainting (and flickering) wholesale.
  // Raw send + paced delay (not this.write) — the per-write failure wait would
  // make a ~250-pixel frame take minutes.
  async drawPixels(changes, pace = 40) {
    for (const p of changes) {
      this.send(`write "${toWriteArg(PROTO.setPixel(p.x, p.y, p.r, p.g, p.b))}" 0 command`);
      await delay(pace);
    }
  }
...
// ── 32x32 framebuffer → iDotMatrix graffiti (per-pixel) draw ─────────────────
...
// Days until a reset timestamp (ceil to whole days), or null. Weekly rows use it.
function daysLeft(ms) {
  if (ms == null) return null;
  return Math.max(0, Math.ceil((ms - Date.now()) / 86400000));
}

// Dense 4-row readout. A 2px service-color stripe runs down the left edge
// (Claude=coral on the top half, Codex=teal on the bottom); each row carries a
// "5h"/"7d" window label + % (severity color) + (5h: mini bar | weekly: days-
// to-reset, blue). Service identity is the stripe; the label is just the window.
function renderQuotaDetail(claude, codex, W = 32, H = 32) {
  ...
  // 2px service-color stripe down the left edge, spanning a service's two rows.
  const stripe = (yTop, yBot, color) => { for (let y = yTop; y <= yBot; y++) { set(0, y, color); set(1, y, color); } };
  // One window row: "5h"/"7d" label (cols 3-13), % (15-25), then 5h bar | weekly days (27-31).
  const row = (y0, weekly, win) => {
    const label = weekly ? '7d' : '5h';
    if (!win) { drawText(set, 3, y0, label, [70, 70, 70]); drawText(set, 15, y0, '--', [70, 70, 70]); return; }
    const pct = weekly ? win.pWk : win.p5h;
    const col = severityColor(pct);
    drawText(set, 3, y0, label, [120, 120, 120]);  // window label
    drawText(set, 15, y0, num2(pct), col);          // percent
    if (weekly) {
      const d = daysLeft(win.rWk);
      drawText(set, 27, y0, d == null ? '?' : String(Math.min(d, 9)), [0, 170, 255]); // days to reset
    } else {
      const fill = Math.round(clamp(pct, 0, 100) / 100 * 5);
      for (let i = 0; i < 5; i++) for (let dy = 2; dy < 6; dy++) set(27 + i, y0 + dy, i < fill ? col : [22, 22, 22]);
    }
  };
  const CLAUDE = [230, 120, 90], CODEX = [0, 190, 170];
  stripe(0, 14, CLAUDE); row(0, false, claude); row(8, true, claude);
  stripe(16, 30, CODEX); row(16, false, codex); row(24, true, codex);
  return buf;
}

// Compare a freshly-rendered RGB framebuffer against what's currently on the
// panel; return only the pixels that differ as {x,y,r,g,b} (including any going
// back to black). Pure — the caller streams these via graffiti setPixel and
// keeps its own `prev` in sync. A null `prev` is treated as all-black, so the
// first frame draws only the lit pixels rather than all 1024.
export function diffFrame(fb, prev, W = 32, H = 32) {
  const changes = [];
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      const i = (y * W + x) * 3;
      const r = fb[i], g = fb[i + 1], b = fb[i + 2];
      const pr = prev ? prev[i] : 0, pg = prev ? prev[i + 1] : 0, pb = prev ? prev[i + 2] : 0;
      if (r !== pr || g !== pg || b !== pb) changes.push({ x, y, r, g, b });
    }
  }
  return changes;
}
...
async function main() {
  ...
  if (opts.dryRun) {
    if (opts.quota) {
      ...
      const lit = diffFrame(fb, null).length;
      console.log(`${lit} lit px — first-frame draw ≈ ${(lit * opts.pace / 1000).toFixed(1)}s @ ${opts.pace}ms/px; later refreshes redraw only changed px`);
      console.log('rows: top=Claude (coral stripe), bottom=Codex (teal stripe); 5h/7d windows; weekly right digit = days to reset:');
  ...
  try {
    ...
    // Graffiti-diff dashboard: clear once, enter DIY draw mode, then only ever
    // poke individual pixels. Staying in one mode (no image upload) is what kills
    // the whole-screen flicker; `prevFb` mirrors the panel so each refresh redraws
    // just the pixels that changed.
    let prevFb = null;
    const showQuota = async () => {
      const { claude, codex } = await fetchQuota();
      const fb = renderQuotaDetail(claude, codex);
      const changes = diffFrame(fb, prevFb);
      await bt.drawPixels(changes, opts.pace);
      prevFb = fb;
      log(`dashboard drawn (${changes.length} px changed)`);
    };
    ...
    if (opts.off) {
      ...
      // Watch for BLE drops during the live loop. bluetoothctl keeps running when
      // the *device* disconnects, so without this our writes fail silently forever.
      const macUp = mac.toUpperCase();
      let linkDown = false;
      bt.onLine((line) => { if (isDisconnected(line) && line.toUpperCase().includes(macUp)) linkDown = true; });

      // Re-establish the link, re-select FA02, clear, and force a full redraw.
      // Infinite retry with backoff (2s→…→30s) — a wall dashboard should self-heal.
      const reconnect = async () => {
        log('link dropped — reconnecting…');
        let wait = 2000;
        while (!bt.closed) {
          try {
            await bt.connect(mac, opts.timeout * 1000, 3, false);
            await bt.selectChar(CHAR_WRITE, opts.timeout * 1000);
            await bt.write(PROTO.fill(0, 0, 0));
            await bt.write(DIY_ON);
            prevFb = null;        // panel is blank after a drop → redraw everything
            linkDown = false;
            await showQuota();
            log('✓ reconnected');
            return;
            ...
            if (bt.closed) return;
            log(`reconnect failed (${e.message}); retry in ${wait / 1000}s`);
            await delay(wait);
      ...
      await bt.write(PROTO.fill(0, 0, 0)); // clear any stale frame → matches prevFb=null (black)
      await bt.write(DIY_ON);              // enter graffiti draw mode once, up front
      await showQuota();
      // Live heartbeat: a top-right dot (31,0 — clear of the content) blinks to
      // show real-time comms; data refreshes every `refresh` seconds. It's now in
      // the same graffiti mode as the dashboard, so it no longer repaints the
      // panel. --beats N runs N blinks then exits.
      const beatMs = 600;
      ...
      for (let b = 0; b < maxBeats; b++) {
        if (linkDown) { await reconnect(); if (bt.closed) break; sinceRefresh = 0; continue; }
        const on = b % 2 === 0;
        try {
          await bt.writeNoWait(PROTO.setPixel(31, 0, ...(on ? [0, 200, 255] : [6, 6, 6])));
        } catch (e) {
          if (bt.closed) { log('bluetoothctl exited — stopping'); break; }
          throw e;
        }
        await delay(beatMs);
        sinceRefresh += beatMs;
